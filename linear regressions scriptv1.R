...
#-----------------Exploring linear relationship using GGPLOT
...
top10ghg_train %>%
  ...
  write.csv("correlation_analysis.csv", row.names = FALSE)


#-----------Fit Linear Regression Models- R2 and Residuals----
library(tidyverse)
library(broom)

# ______ calculate and extract residuals for a linear regression model 
...
analysis1 %>%
  ...
  write.csv("Residualsvalues.csv", row.names = FALSE)    # Save to CSV

#________________Extracting coefficients (intercept & Slope)___________
...
#2. Unnest Residuals To visualize residuals, unnest the residuals into a single data frame:
residuals1 <- residuals0 %>%
  unnest(residuals)

#Residuals Visualize Residuals Per Sector
#3. Plot Residuals Per Sector Use ggplot2 to visualize residuals for each sector:
residuals1 %>%
...
future_predictions <- models %>%
  mutate(
    predictions = map(model, ~predict(.x, newdata = future_years) %>%
                        data.frame(year = future_years$year, Emissions = .))
  ) %>%
  select(Sector, predictions) %>%
  unnest(predictions)  # Combine all sector predictions

#exploring
glimpse(future_predictions)

#نفس الي فوقها
#Make predictions for future years  (Strong, Moderate, Weak) using pipe
...
#Sectors Strong and moderate sectors (excluding Weak)
...
#--------------------Plotting ----------------------
#-______Combine predictions with actual test data
...
#_______Ploting the historical and the predicted values >0 (All sectors)
test_predictions_lr %>% 
  filter(Emissions>0) %>%
...
evaluation_metrics %>%
  ...
  distinct() %>%
  write.csv("evaluation_metrics.csv", row.names = FALSE)
  #view()
